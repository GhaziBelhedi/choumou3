@php
    ...
    $currentType = old('type', $product->type ?? 'livre');
@endphp

<div class="form-grid form-grid-2">
    <div class="field">
        <label class="field__label" for="type">Type de produit</label>
        <select class="select" id="type" name="type" required data-product-type-select>
    ...
    <div class="field">
        ...
        <input class="input @error('title') has-error @enderror" type="text" id="title" name="title" value="{{ old('title', $product->title ?? '') }}" required>
...
<div class="field">
    ...
    <textarea class="textarea @error('description') has-error @enderror" id="description" name="description" rows="5" required>{{ old('description', $product->description ?? '') }}</textarea>
    @error('description') <span class="field__error">{{ $message }}</span> @enderror
</div>

<div class="form-grid form-grid-2" data-book-fields style="{{ $currentType === 'fourniture' ? 'display:none' : '' }}">
    <div class="field">
        <label class="field__label" for="author">Auteur</label>
        <input class="input @error('author') has-error @enderror" type="text" id="author" name="author" value="{{ old('author', $product->author ?? '') }}">
    ...
    <div class="field">
        ...
        <input class="input @error('isbn') has-error @enderror" type="text" id="isbn" name="isbn" value="{{ old('isbn', $product->isbn ?? '') }}">
    ...
    <div class="field">
        ...
        <select class="select" id="language" name="language">
            @foreach (['fr' => 'Français', 'ar' => 'Arabe', 'en' => 'Anglais'] as $value => $label)
                <option value="{{ $value }}" @selected(old('language', $product->language ?? 'fr') === $value)>{{ $label }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div class="field">
        ...
        <input class="input" type="number" id="pages" name="pages" min="1" value="{{ old('pages', $product->pages ?? '') }}">
    </div>
...
<div class="form-grid form-grid-2">
    <div class="field">
        <label class="field__label" for="sku">SKU / Référence <span class="text-faint">(optionnel)</span></label>
    ...
    <div class="field">
        ...
        <input class="input @error('price') has-error @enderror" type="number" step="0.01" min="0" id="price" name="price" value="{{ old('price', $product->price ?? '') }}" required>
    ...
    <div class="field">
        ...
        <input class="input @error('compare_at_price') has-error @enderror" type="number" step="0.01" min="0" id="compare_at_price" name="compare_at_price" value="{{ old('compare_at_price', $product->compare_at_price ?? '') }}">
    ...
    <div class="field">
        ...
        <input class="input @error('stock_quantity') has-error @enderror" type="number" min="0" id="stock_quantity" name="stock_quantity" value="{{ old('stock_quantity', $product->stock_quantity ?? 0) }}" required>
        @error('stock_quantity') <span class="field__error">{{ $message }}</span> @enderror
    </div>

    <div class="field" style="justify-content:center">
        <label class="checkbox-row"><input type="checkbox" name="is_active" value="1" @checked(old('is_active', $product->is_active ?? true))> Actif (visible sur le site)</label>
        <label class="checkbox-row"><input type="checkbox" name="is_featured" value="1" @checked(old('is_featured', $product->is_featured ?? false))> Mettre en avant (coup de cœur)</label>
    </div>

    <div class="field">
        <label class="field__label" for="deal_ends_at">Deal du jour — mis en avant jusqu'au <span class="text-faint">(optionnel)</span></label>
        <input class="input @error('deal_ends_at') has-error @enderror" type="datetime-local" id="deal_ends_at" name="deal_ends_at" value="{{ old('deal_ends_at', optional($product?->deal_ends_at)->format('Y-m-d\TH:i')) }}">
        <span class="field__hint">Si renseigné et dans le futur, ce produit peut apparaître comme « Deal du jour » sur la home (le plus proche de l'échéance est affiché).</span>
...
<div class="field">
    ...
    <div class="flex" style="gap:var(--space-4);flex-wrap:wrap;margin-top:var(--space-2)">
        @php $productCategoryIds = old('categories', ($product?->categories->pluck('id')->all()) ?? []); @endphp
        @foreach ($categories as $category)
            <label class="checkbox-row">
                <input type="checkbox" name="categories[]" value="{{ $category->id }}" @checked(in_array($category->id, $productCategoryIds))>
                {{ $category->name }}
            </label>
        @endforeach
    </div>
...
<div class="form-grid form-grid-2">
    <div class="field">
        ...
        @if (! empty($product?->cover_path))
            <img src="{{ $product->coverUrl() }}" alt="Couverture actuelle de {{ $product->title }}" style="width:80px;height:112px;object-fit:cover;border-radius:var(--radius-sm);margin-bottom:var(--space-2)">
        @endif
            ...
            <div class="image-preview-grid">
                @foreach ($product->images as $image)
                    <div style="position:relative">
                        ...
                        <form method="POST" action="{{ route('admin.produits.images.destroy', $image) }}" style="position:absolute;top:2px;right:2px" onsubmit="return confirm('Supprimer cette image ?')">
                            @csrf @method('DELETE')
